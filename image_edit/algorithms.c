...
void image_convolution_bw(unsigned char ***image_in, unsigned char ***image_out, double **w, int size, double factor, double bias) {
    ...
    for (i = offset; i < lines - offset; i++) {
        for (j = offset; j < columns - offset; j++) {
            t = 0.0;
            for (k = 0; k < size; k++) {
                for (l = 0; l < size; l++) {
                    t = t + (*image_in)[i + k - offset][j + l - offset] * w[k][l]; // Image Convolution with one mask
                }
            }
...
void image_convolution_2d_bw(unsigned char ***image_in, unsigned char ***image_out, double **wx, double **wy, int size, double factor, double bias) {
    ...
    for (i = offset; i < lines - offset; i++) {
        for (j = offset; j < columns - offset; j++) {
            ...
            for (k = 0; k < size; k++) {
                for (l = 0; l < size; l++) {
                    tx = tx + (*image_in)[i + k - offset][j + l - offset] * wx[k][l]; // Image Convolution with one mask at X-Axis
                }
            }
...
void image_convolution_color(
    ...
    for (i = offset; i < lines - offset; i++) {
        for (j = offset; j < columns - offset; j++) {
            ...
            for (k = 0; k < size; k++) {
                for (l = 0; l < size; l++) {
                    tR = tR + (*image_in_r)[i + k - offset][j + l - offset] * w[k][l]; // Image Convolution with one mask
                    tG = tG + (*image_in_g)[i + k - offset][j + l - offset] * w[k][l]; // Image Convolution with one mask
                    tB = tB + (*image_in_b)[i + k - offset][j + l - offset] * w[k][l]; // Image Convolution with one mask
                }
            }
...
void image_convolution_2d_color(
    ...
    for (i = offset; i < lines - offset; i++) {
        for (j = offset; j < columns - offset; j++) {
            ...
            for (k = 0; k < size; k++) {
                for (l = 0; l < size; l++) {
                    txR = txR + (*image_in_r)[i + k - offset][j + l - offset] * wx[k][l]; // Image Convolution with one mask at X-Axis
                    txG = txG + (*image_in_g)[i + k - offset][j + l - offset] * wx[k][l]; // Image Convolution with one mask at X-Axis
                    txB = txB + (*image_in_b)[i + k - offset][j + l - offset] * wx[k][l]; // Image Convolution with one mask at X-Axis
                    tyR = tyR + (*image_in_r)[i + k - offset][j + l - offset] * wy[k][l]; // Image Convolution with one mask at Y-Axis
                    tyG = tyG + (*image_in_g)[i + k - offset][j + l - offset] * wy[k][l]; // Image Convolution with one mask at Y-Axis
